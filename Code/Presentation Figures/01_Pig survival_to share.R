...
survcurv <- ggsurvplot(mod1, data = survdata2,
                       palette = viridis(n = 3, option = "G", begin = 0.8, end = 0.2),
                       legend = "right",
...
png(filename = "../Figures/pigsurvivialcurve2.png", width = 11, height = 5.5,
    units = "in",res = 300)
survcurv
dev.off()
...
csmplot <- ggplot(temp2, aes(x = Cause, y = TotalMorts, fill = age2))+
  geom_bar(stat = "identity")+
  facet_wrap(~ age2)+
  scale_fill_viridis_d(name = "Age class",option = "mako",begin = 0.2,end = 0.94,direction = -1)+
  theme_classic()+
...
png(filename = "../Figures/pigcsm2.png", width = 11, height = 5.5,
    units = "in",res = 300)
csmplot
dev.off()
